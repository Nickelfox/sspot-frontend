...
export const getCustomDate = (schedulerData, num, date = undefined) => {
  ...
  let startDate =
      num === 0 ? selectDate : schedulerData.localeDayjs(new Date(selectDate)).add(2 * num, "days"),
  ...
  if (viewType === ViewType.Custom1) {
    ...
    startDate =
      num === 0 ? monday : schedulerData.localeDayjs(new Date(monday)).add(2 * num, "weeks")
    ...
    startDate =
      num === 0
        ? firstDayOfMonth
        : schedulerData.localeDayjs(new Date(firstDayOfMonth)).add(2 * num, "months")
