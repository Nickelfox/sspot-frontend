...
export default class SchedulerData {
  ...
  getDateLabel() {
    ...
    let label = !!this.behaviors.getDateLabelFunc
    if (label)
  ...
  updateEventStart(event, newStart, title) {
  ...
  _handleRecurringEvents() {
    ...
    recurringEvents.forEach((item) => {
      ...
      let ruleC = !!rule.origOptions.dtstart
      if (ruleC) {
  ...
  _resolveDate(num, date = undefined) {
    ...
    if (this.viewType === ViewType.Week) {
      ...
      if (this.behaviors.getCustomDateFunc != undefined) {
        ...
        let cellC = !!customDate.cellUnit
        if (cellC) this.cellUnit = customDate.cellUnit
      } else {
        throw new Error(
  ...
  _getEventGroupId(event) {
    let groupCheck = !!event.groupId
    return groupCheck ? event.groupId.toString() : event.id.toString()
  }

  _getEventGroupName(event) {
    let groupCheck = !!event.groupName

    return groupCheck ? event.groupName : event.title
  }
